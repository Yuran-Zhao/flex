...
float convert_from_sci_to_float(char* stream, int size);
...
//exp_num_result = e5 = 10000, result = 12300
      float pre_exp_num = 0, exp_num = 0, exp_num_result = 0, result = 0;
...
int get_numberofdigit(char* stream, int size){
    int bit = 0, pos_e = 0, pos_E = 0, pos_point = 0, cur_pos = 0, digit = 0;
...
void print_result(double result){
	 ...
	 if (final_bit > 0){
	   *out = '.';
	   ...
	   low = result - high;
	   float preci = compute_pow(10, -final_bit);
...
int main() {
	char ch[5] = "1e10";
	float result = compute_expression(ch, 5);
	print_result(result);
	return 0;
}